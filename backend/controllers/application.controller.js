...
export const applyJob = async (req, res) => {
  try {
    ...
    if (existing) return res.status(400).json({ message: "Already applied to this job", success: false });
...
export const getAppliedJobs = async (req, res) => {
  try {
    ...
    if (!applications.length) return res.status(404).json({ message: "No applications found", success: false });
